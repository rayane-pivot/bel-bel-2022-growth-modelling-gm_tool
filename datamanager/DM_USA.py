...
class DM_USA(DataManager):
    ...
    PATH_INNO = (
        "data/Growth Modelling - USA - 2018-2021 - Rate Of Innovation (IRI).xlsx"
    )
    PATH_AandP = (
        "data/Growth Modelling - USA - 2018-2021 - Finance Data (SAP - Flux Rio).xlsx"
    )
    PATH_PROMO_COST = "data/Growth Modelling - USA - 2019-2021 - Promo Cost (Xtel).xlsx"
    PATH_HH_INDEX = (
        "data/Growth Modelling - USA - Consumer Panel -US HH penetration GM.xlsx"
    )
    ...
    def open_excel(self):
        ...
        df_periods = (
            pd.DataFrame(np.sort(df_concat.Date.unique()))
            .reset_index()
    ...
    def fill_df_bel(self):
        ...
        df_bel = (
            self.df[self.df.Brand.isin(self.bel_brands)]
            .groupby(["Date", "Brand"], as_index=False)[
                "Distribution", "Price per volume", "Sales in volume"
            ]
            .agg(
                {
                    "Distribution": "mean",
    ...
    def open_excel_old(self):
        ...
        columns = [
            ...
            "Price per volume with promo",
            "Price per volume",
        ]

        # Subcategories for each category in the dataset
        subcategories = {
            "CLASSIC SPREADS": ["CLASSIC SPREADS"],
            ...
            "SNACK": [
                "ALL OTHER SNACK CHEESE",
                "SNACKING BAR / ROUND",
                "SNACKING CRACKER CUTS",
                "SNACKING CUBE",
                "SNACKING SPREADS",
                "SNACKING STRING & STICK",
            ],
            "SNACKING COMBOS": [
                "ALL OTHER COMBOS",
                "CHEESE COMBO WITH FRESH PRODUCE",
                "CHEESE COMBO WITH MEAT PROTEIN",
                "CHEESE DIPPER COMBOS",
                "CHEESE SNACKING COMBO",
            ],
        }
        ...
        for i in range(1, 14):
            ...
            data = pd.read_excel(self.PATH, sheet_name=str(i), engine="openpyxl").iloc[
                8:, :
            ]
            ...
            data["Brand"] = data["Brand"].apply(
                lambda x: x if x != category else "ALL BRANDS"
            )
            # Get the subcategory name for each row and add a corresponding columns
            data["Sub Category"] = data["Brand"].apply(
                lambda x: [
                    subcategory
            ...
            data["Sub Category"] = data["Sub Category"].apply(
                lambda x: x[0] if len(x) > 0 else "ALL SUB CATEGORIES"
            )
            # Remove subcategory name in brand name
            data["Brand"] = data.apply(
                lambda x: replace_strip(x["Brand"], x["Sub Category"]), axis=1
            )
        ...
        periods = (
            df.groupby("Date", as_index=False).any().reset_index()[["index", "Date"]]
        )
