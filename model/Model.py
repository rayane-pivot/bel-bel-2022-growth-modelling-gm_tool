...
class Model():
    ...
    dict_dates = {"Y1": {"Min": "2019-01-01", "Max": "2020-01-01"}, 
              "Y2": {"Min": "2020-01-01", "Max": "2021-01-01"}, 
              "Y3": {"Min": "2021-01-01", "Max": "2022-01-01"}, 
              "3Y": {"Min": "2019-01-01", "Max": "2022-01-01"}}
    
    def compute_brand_positioning_matrix(self, df, key='Sales in volume'):
        brands_to_remove = ["ALL BRANDS"]#, "ZZ OTHERS", "SS.MARQUE", "UNBRANDED", "PL"

        df_2Y = df[(df['Date'] >= '2019-01-01') & (df['Date'] < '2021-01-01')][~df['Brand'].isin(brands_to_remove)]
        
        #build brand positioning matrix
        brand_pos_matrix = pd.pivot_table(df_2Y, values=key, index=['Brand'], columns=['Category'], aggfunc=np.sum, fill_value=0)
        ...
        cagr_brand_value, cagr_category_value, cagr_brand_volume, cagr_category_volume = self.compute_cagr_arrays(df_2Y, Y_BEG=self.dict_dates['Y1'], Y_END=self.dict_dates['Y2'], NbYears=2)
    ...
    def compute_market_passeport(self, df_full):
        brands_to_remove = ["ALL BRANDS"]#, "ZZ OTHERS", "SS.MARQUE", "UNBRANDED", "PL"
        bel_brands = ['APERICUBE', 'BABYBEL', 'MINI BABYBEL', 'BOURSIN', 'KIRI', 'MAREDSOUS', 'VACHE QUI RIT']
        ...
        for year, interval in self.dict_dates.items():
            if year is not '3Y':
                ...
                for category, group in df.groupby('Category'):
                    ...
                    dict_cat['Count of Competitors'] = group[~group['Brand'].isin(bel_brands)]['Brand'].nunique()
                    ...
                    dict_cat['Bel brand sales'] = group[group['Brand'].isin(bel_brands)]['Sales in volume'].sum()
                    dict_cat['Bel brands market share'] = group[group['Brand'].isin(bel_brands)]['Sales in volume'].sum() / group['Sales in volume'].sum() * 100
